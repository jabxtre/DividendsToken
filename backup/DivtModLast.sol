...
contract DividendsToken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function _RewardsRFee(uint256 rRewardsRFee, uint256 tRewardsRFee) private {
        _rTotal = _rTotal.sub(rRewardsRFee);
        _tRewardsTotal = _tRewardsTotal.add(tRewardsRFee);
    }

    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tRewardsRFee, uint256 tBurn, uint256 tCharity) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rRewardsRFee) = _getRValues(tAmount, tRewardsRFee, tBurn, tCharity, _getRate());
        return (rAmount, rTransferAmount, rRewardsRFee, tTransferAmount, tRewardsRFee, tBurn, tCharity);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tRewardsRFee = calculateRewardFee(tAmount);
        uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tCharity =calculateCharityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tRewardsRFee).sub(tBurn).sub(tCharity);
        return (tTransferAmount, tRewardsRFee, tBurn, tCharity);
    }

    function _getRValues(uint256 tAmount, uint256 tRewardsRFee, uint256 tBurn, uint256 tCharity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rRewardsRFee = tRewardsRFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rCharity = tCharity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rRewardsRFee).sub(rBurn).sub(rCharity);
        return (rAmount, rTransferAmount, rRewardsRFee);
    }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        (
            ...
            uint256 rRewardsRFee,
            uint256 tTransferAmount,
            uint256 tRewardsRFee,
            uint256 tBurn,
            uint256 tCharity
        ) = _getValues(tAmount);
        ...
        _RewardsRFee(rRewardsRFee, tRewardsRFee);
